...
class NASBench(object):
    ...
    def __init__(self, dataset_file, seed=None, verbose=True, progress=True):
    ...
    """
        ...
        if progress:
            to_iterate = tqdm(data)
        else:
            to_iterate = data
        for serialized_row in to_iterate:
            # Parse the data from the data file.
            module_hash, epochs, raw_adjacency, raw_operations, raw_metrics = serialized_row
        ...
        for i, k in enumerate(self.fixed_statistics.keys()):
            self.fixed_statistics[k]['index'] = i
