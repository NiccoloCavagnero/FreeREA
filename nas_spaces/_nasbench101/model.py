...
class Network(nn.Module):
    def __init__(self, spec, config, searchspace=[]):
    ...
    def forward(self, x):
        ...
        if not out_concat:  # empty list
            ...
            if self.spec.matrix[0, self.num_vertices - 1]:
                # NO: "+=" is implemented as in-place operation and sometimes it causes trouble during backward
                # for some reason
                # outputs += self.input_op[self.num_vertices - 1](tensors[0])
                outputs = torch.add(outputs, self.input_op[self.num_vertices - 1](tensors[0]))
