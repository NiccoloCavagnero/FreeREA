...
def vertex_min_path(matrix: np.ndarray) -> np.ndarray:
    # Distance vector (for each node it will contain the minimum path from input)
    d = np.zeros((matrix.shape[0],), dtype=matrix.dtype)
    # Initialization: all nodes directly connected with the input have distance 1
    d[:] = matrix[0]
    # For each possible "depth level"
    for i in range(1, d.shape[0] + 1):
        # d == i is the bitmap of the nodes with distance "i" from input
        # with m[d == i] we get the nodes each of those nodes can reach.
        # We reduce the matrix with logical or, so we get a single-dimension bitmap
        # with all the nodes reachable from the nodes with distance "i" from input.
        # We check that those reachable nodes haven't been reached yet (logical and with d == 0)
        d[np.logical_and(np.logical_or.reduce(matrix[d == i], axis=0), d == 0)] = i + 1
    return d


def vertex_max_path(matrix: np.ndarray) -> np.ndarray:
    # Distance vector (for each node it will contain the length of the maximum path from input)
    d = np.zeros((matrix.shape[0],), dtype=matrix.dtype)
    # Initialization: all nodes directly connected with the input have distance 1
    d[:] = matrix[0]
    # For each possible "depth level"
    for i in range(1, d.shape[0] + 1):
        # d == i is the bitmap of the nodes with longest path "i" from input
        # with m[d == i] we get the nodes each of those nodes can reach.
        # We reduce the matrix with logical or, so we get a single-dimension bitmap
        # with all the nodes reachable from the nodes with longest path "i" from input.
        # We check that those reachable nodes have a shorter path than the current one
        d[np.logical_and(np.logical_or.reduce(matrix[d == i], axis=0), d <= i)] = i + 1
    return d
